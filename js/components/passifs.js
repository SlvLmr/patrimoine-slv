import { formatCurrency, formatPercent, openModal, getFormData, inputField } from '../utils.js?v=10';
...
export function render(store) {
  ...
  return `
    <div class="space-y-6">
      <h1 class="text-xl sm:text-2xl font-bold text-gray-100">Gestion des passifs</h1>

      <!-- KPI -->
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="card-dark rounded-xl p-3 sm:p-5 kpi-card glow-red">
          ...
          <p class="text-xl sm:text-2xl font-bold text-accent-red">${formatCurrency(totalDette)}</p>
        </div>
        <div class="card-dark rounded-xl p-3 sm:p-5 kpi-card">
          <div class="flex items-center gap-3 mb-3">
          ...
          <p class="text-xl sm:text-2xl font-bold text-gray-200">${formatCurrency(totalMensualites)}<span class="text-sm font-normal text-gray-500">/mois</span></p>
        </div>
        <div class="card-dark rounded-xl p-3 sm:p-5 kpi-card">
          <div class="flex items-center gap-3 mb-3">
          ...
          <p class="text-xl sm:text-2xl font-bold ${tauxEndettement > 0.35 ? 'text-accent-red' : 'text-accent-green'}">${formatPercent(tauxEndettement)}</p>
      ...
      <div class="card-dark rounded-xl overflow-hidden">
        <div class="p-3 sm:p-5 border-b border-dark-400/30 flex justify-between items-center">
          ...
          <button id="btn-add-emprunt" class="flex items-center gap-1.5 px-3 py-1.5 bg-accent-red/20 text-accent-red text-xs rounded-lg hover:bg-accent-red/30 transition font-medium">+ Ajouter</button>
        </div>
        ${emprunts.length > 0 ? `
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-dark-800/50 text-gray-500">
              <tr>
                <th class="px-3 sm:px-5 py-3 text-left">Nom</th>
                <th class="px-3 sm:px-5 py-3 text-right">Capital restant</th>
                <th class="px-3 sm:px-5 py-3 text-right hidden sm:table-cell">Taux</th>
                <th class="px-3 sm:px-5 py-3 text-right">Mensualité</th>
                <th class="px-3 sm:px-5 py-3 text-right hidden md:table-cell">Durée restante</th>
                <th class="px-3 sm:px-5 py-3 text-right hidden md:table-cell">Avancement</th>
            ...
            <tbody class="divide-y divide-dark-400/20">
              ...
              <tr class="hover:bg-dark-600/30 transition">
                <td class="px-3 sm:px-5 py-3 font-medium text-gray-200">${e.nom}</td>
                <td class="px-3 sm:px-5 py-3 text-right text-accent-red">${formatCurrency(e.capitalRestant)}</td>
                <td class="px-3 sm:px-5 py-3 text-right text-gray-400 hidden sm:table-cell">${formatPercent(e.tauxInteret || 0)}</td>
                <td class="px-3 sm:px-5 py-3 text-right text-gray-300">${formatCurrency(e.mensualite)}</td>
                <td class="px-3 sm:px-5 py-3 text-right text-gray-400 hidden md:table-cell">${e.dureeRestanteMois || 0} mois</td>
                <td class="px-3 sm:px-5 py-3 text-right hidden md:table-cell">
