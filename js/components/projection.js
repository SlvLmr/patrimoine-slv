import { formatCurrency, formatPercent, computeProjection, inputField, selectField, getFormData, getPlacementGroupKey, openModal, confirmModal, showToast } from '../utils.js?v=20260807d';
import { createChart, COLORS, createVerticalGradient, VIVID_PALETTE, ASSET_COLORS } from '../charts/chart-config.js';
import { openAddPlacementModal, openEditPlacementModal } from './placement-form.js?v=20260807d';
import * as ProjectionEnfants from './projection-enfants.js?v=20260807d';
...
import { getEnfants, childAge, CHILD_COLORS } from './projection-enfants.js?v=20260807d';
    ...
    <div class="space-y-6">
      ...
      <div class="card-dark rounded-xl p-3 sm:p-6">
        <div class="flex items-center gap-1.5 mb-4">
          <h2 class="text-[11px] font-semibold text-gray-200 shrink-0">Répartition des actifs</h2>
          <div class="flex items-center gap-1 flex-1 min-w-0">
            <div class="flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-red-500/8 border border-red-500/25 shrink-0">
              <svg class="w-2.5 h-2.5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg>
              <span class="text-[10px] text-red-400 font-medium">Cash out</span>
              <input type="number" id="param-cashout-year" value="${params.cashOutYear || ''}" min="${currentCalendarYear}" max="${currentCalendarYear + 50}" step="1"
                class="param-input w-14 px-0.5 py-0 text-[11px] bg-transparent border-0 text-red-400 focus:ring-0 text-center font-semibold" placeholder="année">
            </div>
            <div class="flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-purple-500/8 border border-purple-500/25 shrink-0">
              ...
              <input type="number" id="param-retraite-souhaitee" value="${params.ageRetraiteSouhaitee || 60}" min="40" max="70" step="1"
                class="param-input w-8 px-0 py-0 text-[11px] bg-transparent border-0 text-purple-400 focus:ring-0 text-center font-semibold">
            ...
            <div class="flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-orange-500/8 border border-orange-500/25 shrink-0">
              ...
              <input type="number" id="param-fire-depenses" value="${params.fireDepensesMensuelles || 1750}" min="0" max="50000" step="50"
                class="param-input w-12 px-0 py-0 text-[11px] bg-transparent border-0 text-orange-400/80 focus:ring-0 text-center font-semibold">
          ...
          <table class="w-full text-sm table-fixed min-w-[1000px]">
            ...
            <tbody class="divide-y divide-dark-400/20">
              ${snapshots.map((s, sIdx) => {
                ...
                const tipDir = s.annee === 0 ? 'proj-tip-below' : '';
        ...
        <div class="px-3 sm:px-5 pb-5 space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div id="strat-repinvest2026" contenteditable="true" class="editable-block min-h-[48px] p-2 sm:p-3 rounded-lg row-item text-xs text-gray-300 leading-snug whitespace-pre-wrap" data-placeholder="Saisir votre répartition 2026...">${(params.strategie?.repinvest2026 || '').replace(/</g, '&lt;') || '<span class=&quot;text-gray-600 italic pointer-events-none&quot;>Saisir votre répartition 2026...</span>'}</div>
            </div>
            <div>
              <h3 class="text-sm font-semibold text-teal-400 mb-2">Répartition investissements 2027</h3>
              <div id="strat-repinvest2027" contenteditable="true" class="editable-block min-h-[48px] p-2 sm:p-3 rounded-lg row-item text-xs text-gray-300 leading-snug whitespace-pre-wrap" data-placeholder="Saisir votre répartition 2027...">${(params.strategie?.repinvest2027 || '').replace(/</g, '&lt;') || '<span class=&quot;text-gray-600 italic pointer-events-none&quot;>Saisir votre répartition 2027...</span>'}</div>
            </div>
            <div>
              <h3 class="text-sm font-semibold text-orange-400 mb-2">Stratégie Donation</h3>
              <div id="strat-donation" contenteditable="true" class="editable-block min-h-[48px] p-2 sm:p-3 rounded-lg row-item text-xs text-gray-300 leading-snug whitespace-pre-wrap" data-placeholder="Saisir votre stratégie donation...">${(params.strategie?.donation || '').replace(/</g, '&lt;') || '<span class=&quot;text-gray-600 italic pointer-events-none&quot;>Saisir votre stratégie donation...</span>'}</div>
            </div>
            <div>
              <h3 class="text-sm font-semibold text-rose-400 mb-2">Stratégie FIRE</h3>
              <div id="strat-fire" contenteditable="true" class="editable-block min-h-[48px] p-2 sm:p-3 rounded-lg row-item text-xs text-gray-300 leading-snug whitespace-pre-wrap" data-placeholder="Saisir votre stratégie FIRE...">${(params.strategie?.fire || '').replace(/</g, '&lt;') || '<span class=&quot;text-gray-600 italic pointer-events-none&quot;>Saisir votre stratégie FIRE...</span>'}</div>
            </div>
          </div>
        </div>
      </details>`;
...
export function mount(store, navigate) {
  ...
  document.getElementById('btn-export-pdf')?.addEventListener('click', async (e) => {
    ...
    try {
      const { exportProjectionPDF } = await import('../export-pdf.js?v=20260807d');
