            ...
            <tbody class="divide-y divide-dark-400/20">
              ${snapshots.map((s, sIdx) => {
                ...
                const placCell = (gk, extraClass = '') => {
                  ...
                  return `<td class="px-1 py-0.5 text-center text-[11px] text-gray-200 ${bt} ${extraClass} ${val > 0 ? 'proj-tip-wrap' : ''}">${val > 0 ? `${formatCurrency(val)}${tip}` : '<span class="text-gray-700">-</span>'}</td>`;
                };
