import { formatCurrency, formatPercent, computeProjection } from '../utils.js?v=10';
...
export function render(store) {
  ...
  return `
    <div class="space-y-6">
      ...
      ${(() => {
        // Collect all valued items
        const placements = state.actifs.placements || [];
        const immobilier = state.actifs.immobilier || [];
        const epargne = state.actifs.epargne || [];

        // Build per-envelope totals (placements by enveloppe + immobilier + epargne)
        ...
        // Build per-placement totals (individual lines)
        const allPlacements = [];
        placements.forEach(p => {
          const val = Number(p.valeur) || Number(p.apport) || 0;
          if (val > 0) allPlacements.push({ nom: p.nom || 'Sans nom', val });
        });
        immobilier.forEach(i => {
          const val = Number(i.valeurActuelle) || 0;
          if (val > 0) allPlacements.push({ nom: i.nom || 'Bien immo', val });
        });
        epargne.forEach(e => {
          const val = Number(e.solde) || 0;
          if (val > 0) allPlacements.push({ nom: e.nom || 'Epargne', val });
        });

        // Build asset class set from placement categories + immobilier + epargne
        const assetClasses = new Set();
        placements.forEach(p => {
          const cat = p.categorie || '';
          const env = (p.enveloppe || p.type || '').toLowerCase();
          const val = Number(p.valeur) || Number(p.apport) || 0;
          if (val <= 0) return;
          ...
          else if (cat === 'Crypto' || env === 'crypto') assetClasses.add('Crypto');
          else if (env === 'livrets') assetClasses.add('Epargne');
          else if (val > 0) assetClasses.add('Autre');
        });
        if (immoTotal > 0) assetClasses.add('Immobilier');
        if (eparTotal > 0) assetClasses.add('Epargne');

        // Envelope count (distinct envelopes with value > 0)
        const envelopeCount = Object.keys(envelopeTotals).filter(k => envelopeTotals[k] > 0).length;

        // --- SCORING ---
        let scoreEnvConcentration = 30;
        let scorePlacConcentration = 30;
        let scoreAssetDiversity = 0;
        let scoreEnvCount = 0;

        // 1) Envelope concentration (max 30)
        let maxEnvPct = 0;
        let maxEnvName = '';
        if (totalAll > 0) {
          Object.entries(envelopeTotals).forEach(([env, val]) => {
            const pct = val / totalAll;
            if (pct > maxEnvPct) { maxEnvPct = pct; maxEnvName = env; }
          });
          if (maxEnvPct > 0.6) scoreEnvConcentration = Math.round(30 * (1 - (maxEnvPct - 0.6) / 0.4));
          else if (maxEnvPct > 0.4) scoreEnvConcentration = Math.round(30 * (0.7 + 0.3 * (1 - (maxEnvPct - 0.4) / 0.2)));
          else scoreEnvConcentration = 30;
        } else {
          scoreEnvConcentration = 0;
        }
        scoreEnvConcentration = Math.max(0, Math.min(30, scoreEnvConcentration));

        // 2) Single placement concentration (max 30)
        let maxPlacPct = 0;
        let maxPlacName = '';
        if (totalAll > 0 && allPlacements.length > 0) {
          allPlacements.forEach(p => {
            const pct = p.val / totalAll;
            if (pct > maxPlacPct) { maxPlacPct = pct; maxPlacName = p.nom; }
          });
          if (maxPlacPct > 0.5) scorePlacConcentration = Math.round(30 * (1 - (maxPlacPct - 0.5) / 0.5));
          else if (maxPlacPct > 0.25) scorePlacConcentration = Math.round(30 * (0.6 + 0.4 * (1 - (maxPlacPct - 0.25) / 0.25)));
          else scorePlacConcentration = 30;
        } else {
          scorePlacConcentration = 0;
        }
        scorePlacConcentration = Math.max(0, Math.min(30, scorePlacConcentration));

        // 3) Asset class diversity (max 25) — 6 possible types
        const numClasses = assetClasses.size;
        if (numClasses >= 5) scoreAssetDiversity = 25;
        else if (numClasses === 4) scoreAssetDiversity = 20;
        else if (numClasses === 3) scoreAssetDiversity = 15;
        else if (numClasses === 2) scoreAssetDiversity = 10;
        else if (numClasses === 1) scoreAssetDiversity = 5;
        else scoreAssetDiversity = 0;

        // 4) Envelope count (max 15)
        if (envelopeCount >= 4) scoreEnvCount = 15;
        else if (envelopeCount === 3) scoreEnvCount = 12;
        else if (envelopeCount === 2) scoreEnvCount = 8;
        else if (envelopeCount === 1) scoreEnvCount = 4;
        else scoreEnvCount = 0;

        const totalScore = scoreEnvConcentration + scorePlacConcentration + scoreAssetDiversity + scoreEnvCount;

        const scoreColor = totalScore > 70 ? 'text-emerald-400' : totalScore >= 40 ? 'text-amber-400' : 'text-red-400';
        const barColor = totalScore > 70 ? 'bg-emerald-500' : totalScore >= 40 ? 'bg-amber-500' : 'bg-red-500';
        const ringColor = totalScore > 70 ? '#10b981' : totalScore >= 40 ? '#f59e0b' : '#ef4444';
        const ringPct = totalAll > 0 ? totalScore : 0;

        // Insights
        const insights = [];
        if (totalAll > 0) {
          if (maxEnvPct > 0.4) {
            insights.push('Concentration ' + maxEnvName + ' : ' + formatPercent(maxEnvPct));
          }
          if (maxPlacPct > 0.25 && allPlacements.length > 1) {
            insights.push((maxPlacName.length > 20 ? maxPlacName.slice(0, 20) + '...' : maxPlacName) + ' : ' + formatPercent(maxPlacPct) + ' du total');
          }
          if (envelopeCount >= 3) {
            insights.push('Diversification enveloppes : bonne (' + envelopeCount + ')');
          } else if (envelopeCount === 2) {
            insights.push('Diversification enveloppes : correcte (' + envelopeCount + ')');
          } else {
            insights.push('Diversification enveloppes : a ameliorer');
          }
          if (numClasses >= 4) {
            insights.push('Classes d\'actifs : bien diversifie (' + numClasses + ' types)');
          } else if (numClasses <= 2 && numClasses > 0) {
            insights.push('Classes d\'actifs : peu diversifie (' + numClasses + ' type' + (numClasses > 1 ? 's' : '') + ')');
          }
        }
        // Keep max 3
        ...
        <div class="flex items-center gap-6">
          <div class="relative flex-shrink-0">
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <span class="${scoreColor} text-2xl font-bold">${totalScore}</span>
            </div>
          </div>
          <div class="flex-1 min-w-0">
            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-500 mb-3">
              <span>Concentration enveloppe</span><span class="text-right text-gray-400">${scoreEnvConcentration}/30</span>
              <span>Concentration placement</span><span class="text-right text-gray-400">${scorePlacConcentration}/30</span>
              <span>Classes d'actifs</span><span class="text-right text-gray-400">${scoreAssetDiversity}/25</span>
              <span>Nb enveloppes</span><span class="text-right text-gray-400">${scoreEnvCount}/15</span>
            </div>
            ${displayInsights.length > 0 ? `
            <div class="space-y-1 border-t border-dark-400/30 pt-2">
              ${displayInsights.map(i => `<p class="text-xs text-gray-500">${i}</p>`).join('')}
            </div>` : ''}
          </div>
        </div>
      </div>` : '';
